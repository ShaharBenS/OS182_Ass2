...
allocproc(void) {
    ...
    *(uint *) sp = (uint) trapret;
    ...
    //********* CHANGED HERE 2.1.2 *****
    for (int j = 0; j < 32; j++) {
        p->signal_handlers[j] = (void *) SIG_DFL;
    }
    p->pending_signals = 0;
...
kill(int pid, int signum) {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        if (p->pid == pid) {
            ...
            int sig_stop_on = (p->pending_signals & (0x80000000 >> SIGSTOP)) == (0x80000000 >> SIGSTOP);
            int sig_cont_got = (signum == SIGCONT);

            if (sig_stop_on || !sig_cont_got)
...
sighandler_t signal(int signum, sighandler_t handler) {
    ...
    sighandler_t old_handler = p->signal_handlers[signum];
    p->signal_handlers[signum] = handler;
    return old_handler;
}

int SIG_KILL_HANDLER(int pid) {
    struct proc *p;
